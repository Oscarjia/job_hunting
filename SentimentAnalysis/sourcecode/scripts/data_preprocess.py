...
def add_arguments(parser):
    ...
    parser.add_argument("--stopwords_file", type=str, default=None, help="stopwords file")
...
def tokenize(content, stopwords):
    ...
    for para in content.split('\n'):
        para_tokens = []
        words = [word for word in list(jieba.cut(para)) if word.strip() not in stopwords]
...
def process_data(output_file, data_file, stopwords):
    data = []
    with open(output_file,'w') as f:
        with open(data_file,encoding='utf-8-sig') as csvfile:
            reader = csv.DictReader(csvfile)
            for i,item in enumerate(reader):
                content = tokenize(item['content'].strip()[1:-1], stopwords)
